...
\begin{table}[t]
\centering
\caption{\textbf{Sequence length scaling on WikiText-2.} Results are computed from \texttt{results/adaptive/scalablity/paper\_benchmark\_scalability.json}. Throughput is in tokens/s (mean\(\pm\)std across prompts), and speedup is relative to AR at the same \(T\).}
...
\begin{table}[t]
\centering
\caption{\textbf{Parameter sensitivity on WikiText-2 (\(T=500\)).} Results are computed from \texttt{results/adaptive/sensitivity/paper\_benchmark\_sensitivity.json}. Speedup is relative to AR under the same setting.}
