...
\label{fig:algo}
\KwIn{Prefix tokens \(x_{1:t}\); target KV cache \(\mathcal{K}_T\); last target logits on the prefix \(\mathbf{s}_{\text{last}}\);
...
\BlankLine
\Comment*[l]{Record prefix cache length for later rollback}
\(\ell \leftarrow \textsc{SeqLen}(\mathcal{K}_T)\).\;

\BlankLine
\Comment*[l]{(A) Draft a token tree with pruning using the draft model \(M_D\)}
\(\mathcal{T} \leftarrow \emptyset\).\;
Run \(M_D\) on \(x_{1:t}\) to obtain next-token logits; let \(u_0\) be the \(\top 1\) token.\;
Add \(u_0\) as the root of \(\mathcal{T}\); set active leaves \(\mathcal{A}\leftarrow\{u_0\}\).\;
\For(\Comment*[f]{level-by-level expansion}){\(d=1\) \KwTo \(D\)}{
  \If(\Comment*[f]{stop if budget reached or nothing to expand}){\(|\mathcal{T}|\ge N_{\max}\) \textbf{or} \(|\mathcal{A}|=0\)}{\textbf{break}}
  \(\mathcal{A}'\leftarrow \emptyset\).\;
  \ForEach{\(u\in\mathcal{A}\)}{
    \If(\Comment*[f]{V2 pruning: skip expanding low-probability branches}){\(\bar{\ell}_u < \log\tau\)}{\textbf{continue}}
    Do a cached one-token forward pass of \(M_D\) from node \(u\) and take \(\top B\) candidates.\;
    Add up to \(B\) children (stop if \(|\mathcal{T}|=N_{\max}\)); add created children to \(\mathcal{A}'\).\;
  }
  \(\mathcal{A}\leftarrow \mathcal{A}'\).\;
}

\BlankLine
\Comment*[l]{(B) Verify all remaining nodes in one target forward pass with tree attention}
Serialize nodes of \(\mathcal{T}\) in BFS order into tokens \(\mathbf{z}_{1:n}\).\;
Build the tree causal mask \(\mathbf{A}\) (prefix + ancestors only).\;
Run \(M_T(\mathbf{z}_{1:n}, \mathbf{A}, \mathcal{K}_T)\) to obtain logits \(\mathbf{s}_{1:n}\) and greedy predictions \(\hat{y}(u)=\arg\max \mathbf{s}_{\text{pos}(u)}\).\;

\BlankLine
\Comment*[l]{(C) Select the longest greedy-consistent path and commit (+ bonus token)}
\(\hat{y}_{t+1}\leftarrow \arg\max \mathbf{s}_{\text{last}}\).\;
Find the longest root-to-leaf path \(P\) such that the root token equals \(\hat{y}_{t+1}\) and each child token matches its parent's greedy prediction \(\hat{y}(\text{parent})\).\;
\eIf(\Comment*[f]{if even the root mismatches}){\(P=\emptyset\)}{
  \(y_{t+1:t+L}\leftarrow [\hat{y}_{t+1}]\).\;
}{
  \(y_{t+1:t+L}\leftarrow\) tokens on \(P\).\;
  Append one \emph{bonus} token equal to the target greedy prediction after the last matched node.\;
}

\BlankLine
\Comment*[l]{Update cache by rollback + rebuild on committed tokens}
\(\mathcal{K}_T \leftarrow \textsc{Crop}(\mathcal{K}_T,\ell)\).\;
Run \(M_T\) on \(y_{t+1:t+L}\) with cache \(\mathcal{K}_T\) to rebuild \(\mathcal{K}_T\).\;
\Return \(y_{t+1:t+L}\).\;
\end{algorithm}
