...
\label{main-results}
Table~\ref{tab:main-results} reports end-to-end throughput on WikiText-2 and PG-19 for \(T=1500\) token generation. Across both datasets, DynaTree yields the highest throughput and improves over both autoregressive decoding and linear speculative decoding. Compared with a tuned fixed tree, DynaTree provides an additional gain (e.g., 220.1~t/s vs.\ 202.3~t/s on WikiText-2), indicating that adapting the draft structure to local model uncertainty improves the draft--verify trade-off beyond a static configuration.

\begin{table}[t]
\centering
\caption{\textbf{Main results (\(T=1500\)).} Throughput (t/s) and speedup relative to autoregressive decoding on WikiText-2 and PG-19. For WikiText-2, Linear uses \(K=8\) and the fixed-tree baseline uses the best configuration from the sweep (\(D=8,B=3,\tau=0.1\)); for PG-19, we use \(K=5\) and \(D=5,B=2\). Values are mean\(\pm\)std over prompts (excluding warmup).}
...
Linear Spec & 205.3\(\pm\)33.4 & 1.54\(\times\) & 154.1\(\pm\)25.4 & 1.15\(\times\) \\
Fixed Tree & 202.3\(\pm\)42.1 & 1.52\(\times\) & 183.7\(\pm\)24.8 & 1.37\(\times\) \\
\textbf{DynaTree} & \textbf{220.1\(\pm\)22.2} & \textbf{1.65\(\times\)} & \textbf{192.1\(\pm\)34.2} & \textbf{1.44\(\times\)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{\textbf{Latency metrics (\(T=1500\)).} We report TTFT (latency to first output token) and TPOT (average per-token latency) on WikiText-2 and PG-19. For WikiText-2, Linear uses \(K=8\) and the fixed-tree baseline uses \((D=8,B=3,\tau=0.1)\); for PG-19, we use \(K=5\) and \(D=5,B=2\). Values are mean\(\pm\)std over prompts (excluding warmup).}
...
\midrule
AR & 19.6\(\pm\)6.0 & 7.49\(\pm\)0.05 & 20.0\(\pm\)4.5 & 7.46\(\pm\)0.04 \\
Linear Spec & 14.8\(\pm\)4.2 & 5.03\(\pm\)1.07 & 10.5\(\pm\)1.9 & 6.71\(\pm\)1.42 \\
Fixed Tree & 14.8\(\pm\)4.3 & 5.26\(\pm\)1.61 & 10.4\(\pm\)0.4 & 5.54\(\pm\)0.75 \\
\textbf{DynaTree} & 14.6\(\pm\)4.0 & \textbf{4.58\(\pm\)0.46} & 9.9\(\pm\)0.6 & \textbf{5.38\(\pm\)1.03} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Throughput and speedup across datasets (\(T=1500\)).} Each method is shown with two bars (WikiText-2 vs.\ PG-19). On WikiText-2, the fixed-tree baseline uses the best configuration from the sweep (\(D=8,B=3,\tau=0.1\)).}
