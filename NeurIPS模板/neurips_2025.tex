...
\begin{itemize}
  ...
  \item \textbf{Fixed Tree:} A static tree speculative decoder with fixed depth \(D\), fixed branching factor \(B\), and node budget \(N_{\max}\), representing a non-adaptive tree baseline. The exact fixed-tree configuration can vary across datasets and analyses; we report the configuration used wherever results are shown.
...
We use a static-tree speculative decoder with a fixed \((D,B,\tau)\) configuration and node budget \(N_{\max}=256\). Since the paper includes multiple workloads (datasets and analysis sections), the fixed-tree configuration is not assumed to be identical everywhere; instead, we report the exact configuration used in each table/figure. For example, Table~\ref{tab:main-results} uses a tuned fixed tree (\(D=8,B=3,\tau=0.1\)), while the progressive ablation in Table~\ref{tab:ablation} uses a simpler fixed-tree backbone (\(D_0=5,B=2\)) for component attribution. We additionally report a fixed-tree hyperparameter sweep under the paper protocol in Appendix~\ref{app:additional-analyses} to verify that the static-tree baseline used in the main benchmark is reasonably tuned.
\paragraph{DynaTree.}
