...
\usepackage{listings}       % pseudocode / code blocks (portable)

% Pseudocode style (portable alternative to algorithm2e/algorithmicx)
...
# Algorithm 1: One DynaTree iteration (greedy-consistent)
# Input : prefix x[1:t], target cache K_T, last_target_logits,
#         depth D, branch factor B, threshold tau, node budget Nmax
# Output: committed tokens (len L) and updated K_T

orig_len <- seq_len(K_T)

# (A) Draft a token tree with pruning (draft model M_D)
T <- init_tree()
root <- top1( M_D(x[1:t]) )          # first draft token
add_root(T, root)
active <- {(root, cache_after_prefix)}
for depth = 1..D:
    if active is empty or |T| >= Nmax: break
    new_active <- {}
    for (u, cache_u) in active:
        if cum_logp(u) < log(tau): continue      # prune this branch (V2)
        logits, cache_next <- M_D(token(u), past=cache_u)
        cand_tokens <- topB(logits, B)
        for v in cand_tokens:
            if |T| >= Nmax: break
            child <- add_child(T, u, v)
            new_active add (child, cache_next)
    active <- new_active

# (B) Verify all remaining nodes in one target forward pass (target model M_T)
tokens <- BFS_flatten(T)                          # length n
mask   <- tree_causal_mask(T, prefix_len=seq_len(K_T))
logits_T, verify_cache <- M_T(tokens, mask, past=K_T)
preds <- argmax_vocab(logits_T)                   # greedy top-1 per node

# (C) Longest greedy-consistent path + bonus token
first <- argmax_vocab(last_target_logits)         # prefix greedy next token
path  <- longest_match_path(T, preds, first)      # as in DFS in code
if path is empty:
    accepted <- [first]
else:
    accepted <- tokens_on(path)
    bonus <- preds[pos(last_node(path))]
    accepted append bonus

# Update cache and return
K_T <- crop(K_T, orig_len)
K_T, last_target_logits <- M_T(accepted, past=K_T)
return accepted
\end{lstlisting}
  \end{minipage}
  \caption{\textbf{DynaTree iteration pseudocode.} Portable pseudocode formatting (algorithm packages unavailable in this environment). Each round drafts and prunes a token tree with the draft model, verifies all remaining nodes in parallel with the target model under a tree causal mask, selects the longest greedy-consistent path (plus a bonus token), and rebuilds the target KV cache for the committed tokens.}
  \label{fig:algo}
\end{figure}
