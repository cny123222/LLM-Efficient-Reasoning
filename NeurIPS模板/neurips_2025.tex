...
\begin{table}[t]
\centering
\caption{\textbf{Verification efficiency comparison.} We report tokens committed per iteration, per-token acceptance rate, and average path length for linear and tree-based methods at their optimal configurations on 500-token generation. Despite lower per-token acceptance, DynaTree's multi-path exploration commits substantially more tokens per verification step.}
\label{tab:verification-efficiency}
\begin{tabular}{lccc}
\toprule
Method & Tokens/Iter & Accept. Rate & Avg Path Length \\
\midrule
Linear speculative (K=6) & 4.10 & 0.68 & 4.10 \\
HuggingFace assisted & -- & -- & -- \\
\textbf{DynaTree (D=8,B=3,\(\tau\)=0.03)} & \textbf{6.94} & 0.38 & \textbf{6.94} \\
\bottomrule
\end{tabular}
\end{table}
