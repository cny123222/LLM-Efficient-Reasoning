...
\begin{table}[tbp]
\centering
\caption{\textbf{Main results (\(T=1500\)).} Throughput (tokens/s) and speedup relative to autoregressive decoding on WikiText-2 and PG-19. Values are mean\(\pm\)std over prompts (excluding warmup). For linear speculation, we use \(K=8\) on WikiText-2 and \(K=5\) on PG-19. The fixed-tree baseline uses \(D=8\), \(B=3\), and pruning threshold \(\tau=0.1\).}
