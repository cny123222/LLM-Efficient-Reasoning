...
\begin{figure}[t]
  ...
  \caption{\textbf{Ablation progression on WikiText-2 (\(T=1500\)).} Beyond throughput and verification iterations, we report per-iteration progress (\(\bar{L}\), \(\bar{\ell}\)) and acceptance rate. Dynamic Breadth \& Depth improves throughput by committing longer prefixes per step, and History Adaptation further increases per-step progress and reduces the number of iterations.}
